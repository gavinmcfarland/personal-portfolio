  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-base">
      ...
      <header className="shrink-0 border-b border-line bg-base">
        <div className="mx-auto w-full max-w-6xl px-5 py-4 sm:px-8 sm:py-5">
          <div className="flex items-start justify-between gap-4 sm:gap-6">
            {/* Left: back, title, short description */}
            ...
            {/* Right: visit link — mr clears the fixed theme toggle in the corner */}
            {project.link && (
              <a
                href={project.link}
                target="_blank"
                rel="noopener noreferrer"
                className="group mr-11 mt-0.5 inline-flex shrink-0 items-center gap-1.5 rounded-full border border-line-strong px-3.5 py-1.5 font-sans text-[0.8125rem] font-medium text-ink transition-colors duration-200 hover:bg-surface sm:mr-12 sm:text-[0.875rem]"
              >
                <span className="hidden sm:inline">{project.linkLabel || "Visit"}</span>
