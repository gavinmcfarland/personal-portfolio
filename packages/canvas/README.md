...
| `collideSeparate` | `boolean` | `false` | (`organic` strategy) also prise apart *intentional* overlaps when reflowing. Default `false`: a reflowed object routes around objects it would collide with, but overlaps present in the authored layout are kept (an overlapping pair reflows around unrelated objects while staying overlapped). Set `true` to separate every overlap. |
