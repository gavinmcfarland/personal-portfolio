...
| `collideSeparate` | `boolean` | `false` | (`organic` strategy) push overlapping objects apart when repositioning. Default `false`: reflow only brings out-of-view objects back on-screen and leaves overlaps (authored or resize-induced) intact. Set `true` to flow overflow downward so nothing overlaps. |
