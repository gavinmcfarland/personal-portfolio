...
(() => {
  ...
  function renderNode(n) {
    ...
    el.style.zIndex = n.z;
    const head = (t, ico) => `<div class="plate__bar">${ico ? icon(ico, { size:16 }) : ''}<span>${esc(t)}</span></div>`;

    if (n.type === 'doc') {
      el.innerHTML = `<div class="plate n-doc">${head(n.head, n.ico)}
        <div class="plate__body">${n.html}${
      ...
      el.innerHTML = `<div class="plate n-code">${head(n.head, n.ico)}<pre>${n.text}</pre></div>`;
    } else if (n.type === 'link') {
      el.innerHTML = `
        <a class="plate n-link" href="${esc(n.url)}" target="_blank" rel="noopener noreferrer">
          ${head(n.head, n.ico)}
