/* ══════════════════════════════════════════════════════════════════
   09 · IMPOSITION — the board
   A small pan/zoom canvas, adapted from 07-passepartout and cut to
   this system's cloth: every node is a class already defined in
   system.css (.note, .nd-doc, .nd-code), so the board and the
   document pages cannot drift apart — they are printed from the
   same form.

   The board owns the whole viewport, so it takes every gesture:
   wheel pans, pinch (ctrl+wheel) zooms at the cursor, drag pans,
   arrows pan, + / − zoom, 0 fits. The dot grid is the viewport's
   background, scaled and offset in apply() so it moves with the
...
window.Board = (() => {
  ...
  function nodeEl(n) {
    ...
    if (n.kind === 'note') {
      el.innerHTML = `<div class="note" data-tone="${esc(n.tone)}" style="min-height:8rem">
      ...
      el.innerHTML = `<div class="nd-doc">
        <span class="micro"><span class="ph ph--16" aria-hidden="true"></span>${esc(n.label)}</span>${esc(n.text)}</div>`;
    }
    return el;
  }
  ...
  function mount(root, notes) {
    ...
    const zoomOut = document.getElementById('board-zoomval');
    ...
    function apply() {
      ...
      if (zoomOut) zoomOut.textContent = Math.round(view.scale * 100) + '%';
    }
    ...
    /* Wheel — a trackpad pinch arrives as a wheel event with ctrlKey
       set; that zooms. A plain wheel pans, which is what a full-bleed
       canvas owes the reader. */
