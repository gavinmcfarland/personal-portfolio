/* ══════════════════════════════════════════════════════════════════
   10 · BROADSHEET — the board
   The same small pan/zoom canvas as 09, kept deliberately identical
   in behaviour so the difference between the two sets reads as a
   difference of REGISTER rather than of capability. Every node is a
   class already defined in system.css, so the board follows the
   theme with no redraw and no second palette to keep in sync.

   The board owns the viewport below the bar, so it takes every
  ...
  function nodeEl(n) {
    ...
    if (n.kind === 'note') {
      ...
      el.innerHTML = `<div class="nd-doc">
        <span class="micro"><span class="ph ph--14" aria-hidden="true"></span>${esc(n.label)}</span>${esc(n.text)}</div>`;
    }
    return el;
  }
