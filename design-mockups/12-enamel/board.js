/* ═════════════════════════════════════════════════════════════════
   12 · ENAMEL — the board
   The same pan/zoom canvas as 09–11, identical in behaviour so the
   difference between the sets reads as one of REGISTER rather than
   of capability.

   It needs less special pleading here than it did in 11 · Galley.
   There, colour was rationed to the board alone and arriving at it
   was the point. Here the sheets are already enamelled, so the board
   is simply the surface where the same four fills stop being panels
   and start being objects you can move. Every node is a class from
   system.css §5; there is no second palette.
  ...
  function nodeEl(n) {
    ...
    if (n.kind === 'note') {
      ...
      el.innerHTML = `<div class="nd-doc">
        <span class="mark"><span class="ph ph--12" aria-hidden="true"></span>${esc(n.label)}</span>${esc(n.text)}</div>`;
    }
    return el;
  }
